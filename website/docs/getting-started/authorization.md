...
By default, an `AssertClaims` result is cached for the exact validated token, effective claims, and authorization policy. Middleware instances with the same policy can share that result through the session, while a different or more privileged policy is evaluated separately. A token refresh, a change in effective claims, or a relevant authentication/authorization configuration change creates a new cache identity and forces evaluation again. Set [`CheckOnEveryRequest`](./middleware-configuration.md#authorization) to `true` to bypass this decision cache and evaluate the policy on every request. When using [`AuthorizationHeader`](./middleware-configuration.md#authorization-header) or [`AuthorizationCookie`](./middleware-configuration.md#authorization-cookie), authorization is always evaluated on every request because there is no persistent server session to trust.

:::important
Failed decisions are scoped and cached in the same way as successful decisions. Depending on [`UnauthorizedBehavior`](./middleware-configuration.md#plugin-config-block), following requests for that same token, claims revision, and policy will either get a 403 again or, with `Challenge` configured, make one IDP round trip before returning to the 403 page. A refreshed token or changed effective claims invalidates that result. With `CheckOnEveryRequest` enabled, the decision is not reused.
:::
